...
const History = struct {
    ...
    fn extrapolate(this: *History) !Extrapolation {
        ...
        while (!all_zeros) {
            ...
            const node = try this.values.allocator.create(Stack(std.ArrayList(i32)).Node);
            node.* = Stack(std.ArrayList(i32)).Node{
                .data = values,
                .next = undefined,
            };
            stack.push(node);
            ...
            var i: usize = 1;
            all_zeros = true;
            var next_values = std.ArrayList(i32).init(this.values.allocator);
            while (i < values.items.len) : (i += 1) {
                const next_value: i32 = values.items[i] - values.items[i - 1];
                try next_values.append(next_value);
                if (next_value != 0) {
                    all_zeros = false;
                }
            }
...
pub fn main() !void {
    const input = @embedFile("09_input.txt");
    const solution = try solve(input, .{});
    std.debug.print("{}\n", .{solution});
}
...
fn solve(input: []const u8, options: Options) !Solution {
    ...
    while (lines.next()) |line| {
        ...
        var values = std.ArrayList(i32).init(options.allocator);
        while (chunks.next()) |chunk| {
            const value = try std.fmt.parseInt(i32, chunk, 10);
